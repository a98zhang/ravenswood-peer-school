...
for (col_name in col_names) {
  ...
  caaspp22_ela_filled[name] <- caaspp22_ela_filled[col_name] / caaspp22_ela_filled$`All.Students`
}
write.csv(caaspp22_ela_filled,file='data/caaspp22_ela_demo.csv')
```
...
```{r output}
caaspp22_ela_score <- caaspp22_ela_filled |> left_join(y=caaspp_ela_aggregate, by = c("School.Code" = "School.Code"))

write.csv(caaspp22_ela_score, file='data/caaspp22_ela_score.csv')
```

### next Step on full data
* need to consider how to extract the all number of students efficiently in reshaping: joining as a separate table?
* should the percentage of demographic distribution be calculated on school level of district level? (school level could be too sparse <> district level could lose info)
* quality check: if percs of groups within one criterion category adds to one (is this a problem?)
* n_schools/districts * 46 demographic groups
* other potential data source: teacher data?
...
caaspp22_ela <- caaspp22_ela %>% drop_na()
nearest <- nn2(caaspp22_ela, caaspp22_ela)
```


```{r modeling-bh}
caaspp22_ela[caaspp22_ela$School.Code == 6044309, ]
top10bh <- nearest$nn.idx[8242,]
top10bhdis <-nearest$nn.dists[8242,]
```
