...
caaspp22_math_pivoted <- caaspp22 %>% 
  # Grade 13 means all grades, Test.ID = 2 (Math)
  filter(Grade==13 & Test.ID==2) %>%
...
caaspp22_math_filled <- data.frame(caaspp22_math_pivoted) # duplicate 
  # merge on grade data
  # merge on outcome data

caaspp22_math_filled <- caaspp22_math_filled |>
...
caaspp22_math_filled[caaspp22_math_filled=='*'] <- '1'    # *s replaced by 1
caaspp22_math_filled[is.na(caaspp22_math_filled)] <- '0'  # NA replaced by 0
caaspp22_math_filled <- caaspp22_math_filled %>% mutate_if(is.character, as.numeric)
```


```{r rename-export}
col_names <- names(caaspp22_math_filled)[3:length(names(caaspp22_math_filled))]
for (col_name in col_names) {
  # rename with the prefix - Percentage
  name <- paste0('Perc.', col_name)
  # calculate the percentage (%)
  caaspp22_math_filled[name] <- caaspp22_math_filled[col_name] / caaspp22_math_filled$`All.Students` * 100
}
caaspp22_math_demo <- caaspp22_math_filled %>% select(-all_of(col_names))

write.csv(caaspp22_math_demo,file='data/caaspp22_math_demo.csv')
```

## Outcome Encoding

```{r scores-distance}
caaspp_outcome_math <- caaspp22 |>
...
#Grade 3 distance from score (2436)
grade_3_math <- caaspp_outcome_math |>
  filter(Grade == 3) |>
  mutate(distance_from_score_math = as.numeric(Mean.Scale.Score) - 2436)

#Grade 4 distance from score (2485)
grade_4_math <- caaspp_outcome_math |>
  filter(Grade == 4) |>
  mutate(distance_from_score_math = as.numeric(Mean.Scale.Score) - 2485)

#Grade 5 distance from score (2528)
grade_5_math <- caaspp_outcome_math |>
  filter(Grade == 5) |>
  mutate(distance_from_score_math = as.numeric(Mean.Scale.Score) - 2528)

#Grade 6 distance from score (2552)
grade_6_math <- caaspp_outcome_math |>
  filter(Grade == 6) |>
  mutate(distance_from_score_math = as.numeric(Mean.Scale.Score) - 2552)

#Grade 7 distance from score (2567)
grade_7_math <- caaspp_outcome_math |>
  filter(Grade == 7) |>
  mutate(distance_from_score_math = as.numeric(Mean.Scale.Score) - 2567)

#Grade 8 distance from score (2586)
grade_8_math <- caaspp_outcome_math |>
  filter(Grade == 8) |>
  mutate(distance_from_score_math = as.numeric(Mean.Scale.Score) - 2586)

caaspp_math_score_distance <- rbind(grade_3_math, grade_4_math, grade_5_math, grade_6_math, grade_7_math, grade_8_math) |>
  na.omit()

caaspp_math_aggregate <- caaspp_math_score_distance |>
  group_by(School.Code) |>
  summarise_at(vars(distance_from_score_math), list(name = mean)) |>
  rename(Avg.Score.Dist.Math=name)
...
caaspp22_math_score <- caaspp22_math_demo |> left_join(y=caaspp_math_aggregate, by = c("School.Code" = "School.Code"))

write.csv(caaspp22_math_score, file='data/caaspp22_math_score.csv')
```
...
caaspp22_math <- read_csv('data/caaspp22_math_demo.csv') |>
...
nearest <- nn2(caaspp22_ela, caaspp22_ela, k=11)
```
...
top10_ravenswood <- top10_ravenswood |> 
  ...
  left_join(y=caaspp22_ela_score, by = c("School.Code" = "School.Code"))
...
test_x = test[, -c(1, length(df))]
test_y = test[, length(df)]
...
train_x = train[, -c(1, length(df))]
train_y = train[, length(df)]

val_x = val[, -c(1, length(df))]
val_y = val[, length(df)]
...
```{r LASSO}
mod_cv <- cv.glmnet(x=as.matrix(df[, -c(1, length(df))]), 
                    y=df[, length(df)], 
                    family='gaussian',
                    alpha=1)
...
res.pca <- prcomp(caaspp22_ela, center = FALSE, scale. = FALSE)
summary(res.pca)
...
peer_school_codes <- top10_ravenswood %>% 
  ...
  .$School.Code
cols <- c("skyblue1", "#55AD89", "#EF6F6A")

caaspp22_ela_pca_biplot = as.data.frame(-res.pca$x[,c(2,3)])
caaspp22_ela_pca_biplot <- caaspp22_ela_pca_biplot |>
...
ggplot(caaspp22_ela_pca_biplot, 
       aes(x = PC1, 
           y = PC3,
           color = School.Group
          )) +
  geom_point(#color="cornflowerblue", 
             ...
             aes(alpha = School.Group,
                 size=School.Group)) +
  geom_text(aes(label=ifelse(School.Code==6044309,'bell_haven','')),
            hjust=.2,
            vjust=.2) + 
